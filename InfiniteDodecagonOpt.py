...
# --- CONFIGURATION ---
GRID_SIZE = 35 # The width and height of your graph (should be a multiple of 5 for Toroidal graphs)
MAX_COL_NUMBER = 40 # Max coloring number to attempt
TOROIDAL = False # Select Infinite (Toroidal) or Finite Grid
...
def compute_distances_and_edges(N, toroidal):
    ...
    """
    # 1. Build Adjacency List for the dodecagonal lattice
    adj = {}
    edges = []
    
    for r in range(N):
        for c in range(N):
            # Alternating vertical connections with a gap of 5 to form dodecagons
            if (r + c) % 5 == 0:
                offsets = [(-1, 0), (0, -1), (0, 1)] # Up, Left, Right
            elif (r + c) % 5 == 4:
                offsets = [(1, 0), (0, -1), (0, 1)]  # Down, Left, Right
            else:
                offsets = [(0, -1), (0, 1)]          # Left, Right
...
def build_model(N, toroidal):
    ...
    print(f"[{time.strftime('%H:%M:%S')}] Generating model: {N}x{N} {'Toroidal' if toroidal else 'Finite'} Dodecagon Grid.")
...
def solve():
    if TOROIDAL and GRID_SIZE % 5 != 0:
        print("ERROR: Toroidal layout requires GRID_SIZE to be a multiple of 5 for uniform dodecagons.")
        return
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        print(f"\nSUCCESS: Solution Found for {GRID_SIZE}x{GRID_SIZE} Dodecagon Grid in {elapsed:.2f}s!")
        for r in range(GRID_SIZE):
            ...
            if r < GRID_SIZE - 1:
                vert_str = ""
                for c in range(GRID_SIZE):
                    # In our coordinate grid mapping, down exists if (r+c) % 5 == 4
                    if (r + c) % 5 == 4:
                        vert_str += "|    "
                    else:
                        vert_str += "     "
